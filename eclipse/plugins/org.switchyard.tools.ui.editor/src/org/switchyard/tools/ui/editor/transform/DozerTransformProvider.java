...
public class DozerTransformProvider implements ITransformProvider {
    ...
    public String getName() {
        return Messages.DozerTransformProvider_DozerTransformer_DisplayName;
    }
    ...
    private static final class DozerTransformControl implements ITransformControl {
        ...
        private DozerTransformControl(Composite parent, IContainer container) {
            ...
            label.setText(Messages.DozerTransformProvider_Dozer_File_Label);
            ...
            browseButton.setText(Messages.DozerTransformProvider_Dozer_File_Browse_Button);
            ...
            _createNewFileCheckbox.setText(Messages.DozerTransformProvider_New_File_Checkbox);
        ...
        public IStatus validate() {
            ...
            if (!path.isValidPath(pathString)) {
                return new Status(Status.ERROR, Activator.PLUGIN_ID,
                        Messages.DozerTransformProvider_invalid_path_message);
            }
            try {
                ...
                if (_createNewFileCheckbox.getSelection()) {
                    if (file.exists()) {
                        return new Status(Status.ERROR, Activator.PLUGIN_ID,
                                Messages.DozerTransformProvider_duplicate_file_message);
                    }
                } else if (!file.exists()) {
                    return new Status(Status.ERROR, Activator.PLUGIN_ID,
                            Messages.DozerTransformProvider_file_does_not_exist_message);
                }
                if (!_project.isOnClasspath(file.getParent())) {
                    return new Status(Status.WARNING, Activator.PLUGIN_ID,
                            Messages.DozerTransformProvider_file_not_on_classpath_message);
                }
            } catch (Exception e) {
        ...
        private IStatus validateSelectedTransformers() {
            ...
            for (TransformType transformer : _transforms) {
                if (!NewTransformWizard.isJavaType(transformer.getFrom())
                    ...
                    return new Status(Status.ERROR, Activator.PLUGIN_ID,
                            Messages.DozerTransformProvider_only_supports_java_types_message);
                }
            }
            return Status.OK_STATUS;
        }
        ...
        private void browse() {
            ...
            dialog.setTitle(Messages.DozerTransformProvider_Browse_Dialog_title);
